...
def convert2npy(output_path, flow, scene, config, frame, num_view):
    ...
    for ref_idx in range(num_view):
        ...
        # mask_tmp = cv2.imread('/data/GoPro/videos/teaRoom/sequence/mask/{}-{}.png'.format(ref_idx+1, int(frame[4:])))[:,:,0]
        # mask_tmp = cv2.resize(mask_tmp, (1600, 912))
        # mask_tmp = mask_tmp[8:-8,:]
        # depth_tmp[mask_tmp < 100] = 0
